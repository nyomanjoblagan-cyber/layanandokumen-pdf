...
export default function FillFormPage() {
  ...
  const T = {
    ...
    hero_desc: { 
      id: 'Isi formulir PDF interaktif otomatis. Deteksi kolom teks, centang, dan pilihan.', 
      en: 'Fill interactive PDF forms automatically. Detects text, checkboxes, and selections.' 
    },
    select_btn: { id: 'Pilih File PDF', en: 'Select PDF File' },
    drop_text: { id: 'atau tarik file ke sini', en: 'or drop file here' },
    tab_preview: { id: 'Preview', en: 'Preview' },
    tab_form: { id: 'Isi Data', en: 'Fill Data' },
    no_form_title: { id: 'Tidak Ada Formulir', en: 'No Form Fields' },
    no_form_desc: { id: 'PDF ini statis (tidak ada kolom interaktif). Gunakan menu "Edit PDF" untuk mengetik manual.', en: 'This is a static PDF. Use "Edit PDF" menu to type manually.' },
    ...
    success_desc: { id: 'Formulir berhasil disimpan.', en: 'Form saved successfully.' },
    download_btn: { id: 'Download PDF', en: 'Download PDF' },
    back_home: { id: 'Isi Lagi', en: 'Fill Another' },
  ...
  const processFile = async (uploadedFile: File) => {
        ...
        if (rawFields.length === 0) {
            ...
            const extractedFields: FormField[] = rawFields.map(f => {
                ...
                if (f instanceof PDFTextField) {
                    ...
                    type = 'Dropdown'; value = f.getSelected()[0] || ''; options = f.getOptions();
                } else if (f instanceof PDFRadioGroup) {
                    type = 'Radio'; value = f.getSelected() || ''; options = f.getOptions();
                }
    ...
    } catch (error) { alert("Gagal memproses PDF."); setFile(null); } finally { setIsProcessing(false); }
  };
    ...
    try {
        ...
        fields.forEach(f => {
            try {
                ...
                if (f.type === 'Text' && field instanceof PDFTextField) field.setText(f.value as string);
                else if (f.type === 'CheckBox' && field instanceof PDFCheckBox) f.value ? field.check() : field.uncheck();
                else if (f.type === 'Dropdown' && field instanceof PDFDropdown) field.select(f.value as string);
                else if (f.type === 'Radio' && field instanceof PDFRadioGroup) field.select(f.value as string);
            } catch (e) { console.warn(`Skip field: ${f.name}`); }
        });
  ...
  const resetAll = () => { setFile(null); setPdfUrl(null); setFields([]); setMobileTab(1); };
        ...
        {!file ? (
          ...
          >
             <div className="w-full max-w-[1400px] flex gap-4 xl:gap-8 justify-center items-start pt-4 md:pt-10">
                ...
                <div className="flex-1 max-w-4xl space-y-8 animate-in fade-in zoom-in duration-500">
                    <div className="mb-8 flex justify-center"><AdsterraBanner height={90} width={728} data_key="c0fd3ef02cfd2ffa7fda180dcda83f73" /></div>
                    ...
                    <div className="flex flex-col items-center gap-6 py-4">
                        <button onClick={() => fileInputRef.current?.click()} className="w-full md:w-auto bg-indigo-600 hover:bg-indigo-700 text-white text-lg md:text-xl font-bold py-5 md:py-6 px-10 md:px-16 rounded-xl shadow-xl shadow-indigo-200 hover:shadow-2xl hover:-translate-y-1 transition-all active:scale-95 flex items-center justify-center gap-3">
                           {isProcessing ? <Loader2 className="animate-spin" size={24}/> : <PenTool size={24} />} {isProcessing ? T.processing[lang] : T.select_btn[lang]}
                        </button>
                        <p className="text-slate-400 text-xs md:text-sm font-bold tracking-wide">{T.drop_text[lang]}</p>
                    </div>
                    <div className="mt-10 flex justify-center"><AdsterraBanner height={250} width={300} data_key="56cc493f61de5edcff82fc45841616e5" /></div>
          ...
          <div className="flex-1 flex flex-col items-center justify-center p-6 text-center bg-white overflow-y-auto">
             <div className="w-full max-w-5xl flex gap-8 justify-center items-start pt-10">
                ...
                <div className="flex-1 max-w-xl space-y-8 animate-in slide-in-from-bottom duration-500">
                    <div className="mb-8"><AdsterraBanner height={90} width={728} data_key="c0fd3ef02cfd2ffa7fda180dcda83f73" /></div>
          ...
          <div className="flex flex-col h-full md:flex-row md:h-auto md:p-6 md:gap-6 max-w-7xl mx-auto w-full">
            <div className="md:hidden flex border-b border-slate-200 bg-white sticky top-0 z-20 shrink-0">
               <button onClick={() => setMobileTab(0)} className={`flex-1 py-3 text-xs font-bold uppercase tracking-wider border-b-2 transition-colors ${mobileTab === 0 ? 'border-indigo-600 text-indigo-600' : 'border-transparent text-slate-400'}`}>{T.tab_preview[lang]}</button>
               <button onClick={() => setMobileTab(1)} className={`flex-1 py-3 text-xs font-bold uppercase tracking-wider border-b-2 transition-colors ${mobileTab === 1 ? 'border-indigo-600 text-indigo-600' : 'border-transparent text-slate-400'}`}>{T.tab_form[lang]}</button>
            </div>
            ...
            <div className={`flex-1 flex flex-col h-full bg-slate-100 md:bg-white md:rounded-2xl md:shadow-xl md:border border-slate-200 md:p-8 overflow-hidden relative ${mobileTab === 0 ? 'flex' : 'hidden md:flex'}`}>
                <div className="flex-1 flex items-center justify-center p-4 overflow-auto">
                    <div className="relative shadow-2xl border-4 border-white bg-white max-w-full max-h-full">
                        {previewImage ? <img src={previewImage} alt="Preview" className="max-w-full max-h-[60vh] md:max-h-[600px] object-contain" /> : <div className="w-[400px] h-[600px] flex items-center justify-center"><Loader2 className="animate-spin text-slate-300" size={40}/></div>}
                    </div>
                </div>
            </div>
            ...
            <div className={`w-full md:w-96 bg-white md:rounded-2xl md:shadow-xl md:border border-slate-200 p-6 overflow-y-auto md:h-fit shrink-0 ${mobileTab === 1 ? 'block' : 'hidden md:block'}`}>
                ...
                {hasForm === false ? (
                    <div className="bg-red-50 border border-red-100 rounded-xl p-6 text-center">
                        ...
                        <Link href="/tools/edit-pdf" className="inline-block w-full bg-white border border-red-200 text-red-600 font-bold py-3 rounded-lg text-xs hover:bg-red-50 uppercase tracking-wider">{T.btn_goto_edit[lang]}</Link>
                    </div>
                ) : (
                    <div className="space-y-4">
                        ...
                        <div className="space-y-4 max-h-[400px] overflow-y-auto pr-1 custom-scrollbar">
                            {fields.map((field, idx) => (
                                <div key={idx} className="space-y-1">
                                    <label className="text-[10px] font-bold text-slate-500 uppercase tracking-wide truncate block">{field.name}</label>
                                    {field.type === 'Text' && (
                                        <input type="text" value={field.value as string} onChange={(e) => updateField(idx, e.target.value)} className="w-full border-2 border-slate-200 rounded-lg px-3 py-2 text-sm font-bold text-slate-700 outline-none focus:border-indigo-500 transition-colors" />
                                    )}
                                    {field.type === 'CheckBox' && (
                                        <div onClick={() => updateField(idx, !field.value)} className={`flex items-center gap-3 p-3 rounded-lg border-2 cursor-pointer transition-all ${field.value ? 'border-indigo-500 bg-indigo-50' : 'border-slate-200 hover:border-slate-300'}`}>
                                            <div className={`w-5 h-5 rounded border-2 flex items-center justify-center ${field.value ? 'bg-indigo-500 border-indigo-500' : 'border-slate-300 bg-white'}`}>{field.value && <CheckCircle2 size={14} className="text-white"/>}</div>
                                            <span className="text-xs font-bold text-slate-600">{field.value ? 'Terpilih' : 'Tidak Terpilih'}</span>
                                        </div>
                                    )}
                                    {(field.type === 'Dropdown' || field.type === 'Radio') && (
                                        <select value={field.value as string} onChange={(e) => updateField(idx, e.target.value)} className="w-full bg-slate-50 border border-slate-200 rounded-lg px-3 py-2 text-sm font-bold text-slate-700 outline-none focus:border-indigo-500 cursor-pointer">
                                            {field.value === '' && <option value="" disabled>Pilih...</option>}
                        ...
                        <div className="pt-4 border-t border-slate-100 flex justify-center"><AdsterraBanner height={250} width={300} data_key="56cc493f61de5edcff82fc45841616e5" /></div>
                        <button onClick={handleSave} disabled={isSaving} className="w-full bg-indigo-600 hover:bg-indigo-700 disabled:bg-slate-300 disabled:text-slate-400 text-white font-black py-4 rounded-2xl shadow-lg active:scale-95 transition-all flex items-center justify-center gap-2 mt-2 uppercase text-xs tracking-widest">
                            {isSaving ? <Loader2 className="animate-spin" size={18}/> : <FileText size={18}/>} {isSaving ? T.saving[lang] : T.save_btn[lang]}
                        </button>
                    </div>
                )}
            </div>
