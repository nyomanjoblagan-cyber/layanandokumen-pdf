...
export default function WatermarkPdfPage() {
  ...
  const T = {
    ...
    hero_desc: { 
      id: 'Tambahkan teks tanda air (watermark) ke dokumen PDF Anda. Preview langsung dan posisi akurat.', 
      en: 'Add text watermark to your PDF documents. Live preview and accurate positioning.' 
    },
    ...
    label_pos: { id: 'Posisi', en: 'Position' },
  ...
  // Helper konversi warna Hex ke RGB (0-1 range untuk pdf-lib)
  ...
  // --- LOGIKA SAVE (KALKULASI POSISI PRESISI) ---
  const handleSave = async () => {
    ...
    try {
        ...
        pages.forEach((page) => {
            ...
            // Kalkulasi Y (Vertical) - PDF Lib Y=0 ada di Bawah
  ...
  // --- LOGIKA CSS PREVIEW (MIMIC PDF COORDINATES) ---
  const getPreviewStyles = () => {
    const style: React.CSSProperties = {
        ...
        opacity: opacity,
        color: color,
        fontSize: `${fontSize}px`, // Perlu disesuaikan skalanya dengan layar nanti
        fontWeight: 'bold',
        whiteSpace: 'nowrap',
        fontFamily: 'Helvetica, sans-serif',
        padding: '20px', // PADDING Visual
        pointerEvents: 'none',
    };

    // Flexbox Alignment untuk Preview Container
    ...
    if (position.includes('top')) containerStyle.alignItems = 'flex-start';
    else if (position.includes('bottom')) containerStyle.alignItems = 'flex-end';
    else containerStyle.alignItems = 'center';

    if (position.includes('left')) containerStyle.justifyContent = 'flex-start';
    else if (position.includes('right')) containerStyle.justifyContent = 'flex-end';
    else containerStyle.justifyContent = 'center';

    return { style, containerStyle };
  };

  const { style: textStyle, containerStyle } = getPreviewStyles();
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] text-slate-800 font-sans relative selection:bg-blue-100 selection:text-blue-700 flex flex-col overflow-hidden">
      ...
      <main className="flex-1 relative z-10 flex flex-col h-[calc(100dvh-56px)] md:h-auto">
        ...
        {/* STATE 3: EDITOR (PREVIEW + SETTINGS) */}
        {file && !pdfUrl && (
          <div className="flex flex-col h-full md:flex-row md:h-auto md:p-6 md:gap-6 max-w-7xl mx-auto w-full">
            ...
            <div className={`flex-1 flex flex-col h-full bg-slate-100 md:bg-white md:rounded-2xl md:shadow-xl md:border border-slate-200 md:p-8 overflow-hidden relative ${mobileTab === 0 ? 'flex' : 'hidden md:flex'}`}>
                <div className="flex-1 flex items-center justify-center p-4 overflow-auto">
                    {/* CONTAINER RELATIF UNTUK IMAGE + WATERMARK */}
                    <div className="relative shadow-2xl border-4 border-white bg-white max-w-full overflow-hidden select-none">
                        {previewImage && (
                            <img 
                                src={previewImage} 
                                alt="Preview" 
                                className="max-w-full max-h-[60vh] md:max-h-[600px] object-contain block" 
                            />
                        )}
                        ...
                        <div style={containerStyle}>
                            <div style={{
                                ...textStyle, 
                                // Penyesuaian skala font visual agar mirip dengan PDF (Approximation)
                            ...
                            <div className="grid grid-cols-3 gap-1 w-24 h-24 bg-slate-100 p-1 rounded-lg">
                                {['top-left', 'top', 'top-right', 'left', 'center', 'right', 'bottom-left', 'bottom', 'bottom-right'].map((pos) => (
                                    <button 
                                      ...
                                      className={`rounded hover:bg-blue-100 transition-colors ${position === pos ? 'bg-blue-600 shadow-md' : 'bg-white'}`} 
                                      title={pos}
                                    />
                                ))}
                            </div>
